-- SMALL CAMERA — flexible VAT editing and user-controlled next document numbers.
-- Run once in Supabase SQL Editor after the existing VAT migrations.

ALTER TABLE public.vat_settings
  ADD COLUMN IF NOT EXISTS next_full_number BIGINT NOT NULL DEFAULT 1,
  ADD COLUMN IF NOT EXISTS next_abbreviated_number BIGINT NOT NULL DEFAULT 1;

ALTER TABLE public.vat_settings DROP CONSTRAINT IF EXISTS vat_settings_next_full_number_check;
ALTER TABLE public.vat_settings ADD CONSTRAINT vat_settings_next_full_number_check CHECK (next_full_number >= 1);
ALTER TABLE public.vat_settings DROP CONSTRAINT IF EXISTS vat_settings_next_abbreviated_number_check;
ALTER TABLE public.vat_settings ADD CONSTRAINT vat_settings_next_abbreviated_number_check CHECK (next_abbreviated_number >= 1);
