...
export function downloadImportTemplate() {
  ...
  // Sheet 1: Products
  const pData = [
    ['ชื่อรุ่น *','Serial Number *','ประเภท *','เกรดสภาพ * (1-5)','ราคาซื้อ (บาท) *','สถานะ *','ราคาขาย (บาท)','ช่องทางชำระ','วันที่รับเข้า (DD/MM/YYYY HH.mm)','วันที่ขาย (DD/MM/YYYY HH.mm)','ยอดผ่อนทั้งหมด','ยอดชำระแล้ว','รายละเอียดลูกค้า','หมายเหตุ'],
    ['Fujifilm X100V','AB12345','กล้อง',4,28000,'พร้อมขาย','','','01/04/2568 10.00','','','','','ตัวอย่าง — ลบแถวนี้ออก'],
    ['Sony A7III','CD67890','กล้อง',5,55000,'ขายแล้ว',65000,'โอน','15/03/2568 09.00','20/03/2568 15.30','','','ลูกค้าตัวอย่าง','ตัวอย่าง — ลบแถวนี้ออก'],
  ]
  const ws1 = XLSX.utils.aoa_to_sheet(pData)
  ws1['!cols'] = [20,18,12,16,18,14,18,14,26,26,16,16,28,28].map(w=>({wch:w}))
  XLSX.utils.book_append_sheet(wb, ws1, 'สต็อกสินค้า')

  // Sheet 2: Transactions
  const tData = [
    ['วันที่ * (DD/MM/YYYY HH.mm)','ประเภท * (เงินเข้า/เงินออก)','หมวดหมู่ *','จำนวนเงิน (บาท) *','ช่องทางชำระ','ยอดโอน','ยอดเงินสด','Serial สินค้า','หมายเหตุ'],
    ['01/04/2568 10.00','เงินออก','Buy Stock',28000,'โอน',28000,'','AB12345','ตัวอย่าง — ลบแถวนี้ออก'],
    ['20/03/2568 15.30','เงินเข้า','Sale',65000,'โอน',65000,'','CD67890','ตัวอย่าง — ลบแถวนี้ออก'],
    ['01/04/2568 09.00','เงินออก','Marketing',500,'เงินสด','',500,'','ตัวอย่าง — ลบแถวนี้ออก'],
  ]
  const ws2 = XLSX.utils.aoa_to_sheet(tData)
  ws2['!cols'] = [28,22,16,20,16,14,14,18,30].map(w=>({wch:w}))
  XLSX.utils.book_append_sheet(wb, ws2, 'รายการบัญชี')

  const aData = [
    ['Serial สินค้า *','ชื่ออุปกรณ์เสริม *','ต้นทุน (บาท) *','วันที่เพิ่ม (DD/MM/YYYY HH.mm)'],
    ['AB12345','แบตเตอรี่สำรอง',1200,'02/04/2568 10.00'],
  ]
  const ws3 = XLSX.utils.aoa_to_sheet(aData)
  ws3['!cols'] = [18,28,18,26].map(w=>({wch:w}))
  XLSX.utils.book_append_sheet(wb, ws3, 'อุปกรณ์เสริม')

  const bData = [
    ['เงินโอนคงเหลือ *','เงินสดคงเหลือ *'],
    [100000,5000],
  ]
  const ws4 = XLSX.utils.aoa_to_sheet(bData)
  ws4['!cols'] = [22,22].map(w=>({wch:w}))
  XLSX.utils.book_append_sheet(wb, ws4, 'ยอดเงินคงเหลือ')
