...
export default function AddProduct() {
  ...
  const [form, setForm] = useState({ serial_number: '', model: '', condition: 5, base_cost: '', notes: '' })
  ...
  const save = async () => {
    ...
    try {
      ...
      const { data: p, error } = await supabase.from('products').insert({
        serial_number: form.serial_number.trim(), model: form.model.trim(),
        condition: Number(form.condition), base_cost: cost, total_cost: cost,
        notes: form.notes,
      }).select().single()
            ...
            {[5,4,3,2,1].map(c => (
              <button key={c} onClick={() => F('condition', c)}
                className={`flex-1 py-2 rounded-xl text-sm font-semibold border transition-all
                  ${form.condition === c ? 'bg-brand-dark text-brand-yellow border-brand-dark' : 'bg-white text-gray-500 border-gray-200'}`}>
                {c}
              </button>
            ))}
          </div>
